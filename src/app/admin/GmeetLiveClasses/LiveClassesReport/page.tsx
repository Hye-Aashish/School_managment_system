                  ...
                  <div className="relative">
                    ...
                    <div className={`rounded-lg w-full shadow-lg bg-white dark:bg-darkblack-500 absolute left-0 z-10 top-14 overflow-hidden transition-all ${openFilter === "class" ? "block" : "hidden"}`}>
                      <ul className="max-h-48 overflow-y-auto">
                        {classList.map((cls) => (
                          <li key={cls} className="text-sm text-bgray-900 dark:text-white cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 font-medium" onClick={() => { setSelectedClass(cls); setOpenFilter(null); }}>{cls}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                    ...
                    <div className={`rounded-lg w-full shadow-lg bg-white dark:bg-darkblack-500 absolute left-0 z-10 top-14 overflow-hidden transition-all ${openFilter === "section" ? "block" : "hidden"}`}>
                      <ul className="max-h-48 overflow-y-auto">
                        {sectionList.map((section) => (
                          <li key={section} className="text-sm text-bgray-900 dark:text-white cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 font-medium" onClick={() => { setSelectedSection(section); setOpenFilter(null); }}>{section}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
