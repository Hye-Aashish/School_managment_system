...
export default function ZoomLiveMeetingModal({ isOpen, onClose, onRefresh, editData }: ModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editData ? `/api/zoom-live-meetings/${editData._id}` : "/api/zoom-live-meetings";
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to save Zoom Live Meeting");
      }
    } catch (error) {
      console.error("Error saving Zoom Live Meeting:", error);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-darkblack-600 rounded-lg w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4 text-bgray-900 dark:text-white">
          {editData ? "Edit Zoom Live Meeting" : "Add Zoom Live Meeting"}
        </h2>
