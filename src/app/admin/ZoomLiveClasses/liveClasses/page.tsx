...
import ZoomLiveClassModal from "./ZoomLiveClassModal";
...
export default function ZoomLiveClasses() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this Zoom live class?")) return;
    try {
      await fetch(`/api/zoom-live-classes/${id}`, { method: "DELETE" });
      fetchClasses();
    } catch (error) {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    try {
      const res = await fetch(`/api/zoom-live-classes/${id}`, {
        method: "PATCH",
  ...
  const handleCsvDownload = () => {
    ...
    a.href = url;
    a.download = "zoom-live-classes.csv";
    a.click();
  ...
  const handleExcelDownload = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "zoom-live-classes.xls";
    a.click();
    URL.revokeObjectURL(url);
  };
      ...
      <div className="2xl:flex 2xl:space-x-[48px]">
        <section className="2xl:flex-1 2xl:mb-0 mb-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-bgray-900 dark:text-white">Zoom Live Classes</h1>
            <button
              type="button"
              onClick={handleAdd}
                ...
                <div className="flex-1 border border-transparent focus-within:border-success-300 h-full bg-bgray-200 dark:bg-darkblack-500 rounded-lg px-[18px]">
                  <div className="flex w-full h-full items-center space-x-[15px]">
                    ...
                    <label className="w-full">
                      <input
                        type="text"
                        placeholder="Search Zoom Live Classes..."
                        value={searchQuery}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-bgray-300 dark:border-darkblack-400">
                      {isVisible("classTitle") && <td className="py-4 px-4 text-sm font-semibold text-bgray-600 dark:text-bgray-50">Class Title</td>}
                      ...
                      {isVisible("createdFor") && <td className="py-4 px-4 text-sm font-semibold text-bgray-600 dark:text-bgray-50">Created For</td>}
                  ...
                  <tbody>
                    {isLoading ? (
                      <tr><td colSpan={9} className="text-center py-4 text-bgray-600 dark:text-bgray-50">Loading...</td></tr>
                    ) : filtered.length === 0 ? (
                      <tr><td colSpan={9} className="text-center py-4 text-bgray-600 dark:text-bgray-50"> {searchQuery ? "No matching Zoom classes found." : "No Zoom live classes found."} </td></tr>
                    ) : (
                      filtered.map((item) => (
                        <tr key={item._id} className="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-bgray-100 dark:hover:bg-darkblack-500 transition duration-200">
                          {isVisible("classTitle") && <td className="py-4 px-4 text-sm text-bgray-900 dark:text-bgray-50">{item.classTitle}</td>}
                          ...
                          {isVisible("createdFor") && <td className="py-4 px-4 text-sm text-bgray-900 dark:text-bgray-50">{item.createdFor}</td>}
