...
export default function SendSMS() {
     ...
     const handleSend = async (e: React.FormEvent) => {
          ...
          try {
               ...
               alert("SMS communication deployed successfully!");
          ...
          <div className="flex flex-col space-y-6 px-1">
               <section className="bg-white dark:bg-darkblack-600 rounded-3xl p-8 shadow-sm border border-bgray-200 dark:border-darkblack-400">
                    <div className="flex items-center gap-4 mb-10 pb-6 border-b border-dashed border-bgray-100 dark:border-darkblack-400">
                         ...
                         <div>
                              <h3 className="text-2xl font-black dark:text-white uppercase tracking-tighter leading-none">Institutional SMS Dispatch</h3>
                              <p className="text-[10px] font-bold text-bgray-400 uppercase tracking-widest mt-2">Deploying SMS payloads to institutional demographics</p>
                         </div>
                    </div>
                    ...
                    <form onSubmit={handleSend} className="space-y-8 max-w-4xl">
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-2">
                                   <label className="text-[10px] font-black text-bgray-400 uppercase tracking-widest px-1">Campaign Title *</label>
                                   <input required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full h-14 bg-bgray-50 dark:bg-darkblack-500 rounded-2xl px-6 text-sm font-bold border-none outline-none focus:ring-2 focus:ring-success-300/30" placeholder="Campaign Name..." />
                              </div>
                              <div className="space-y-2">
                         ...
                         <div className="space-y-2">
                              <div className="flex justify-between items-center px-1">
                                   ...
                                   <span className="text-[10px] font-black text-bgray-300 uppercase tracking-tighter italic">{formData.message.length} chars</span>
                              </div>
