...
export default function StaffAttendance() {
     ...
     const saveAttendance = async () => {
          ...
          try {
               ...
               if (res.ok) showToast("Attendance archived successfully", "success");
          } catch (e) {
               console.error(e);
          } finally {
               setSaving(false);
     ...
     return (
          <div className="flex flex-col space-y-6 px-1">
               ...
               <section className="bg-white dark:bg-darkblack-600 rounded-2xl shadow-sm border border-bgray-200 dark:border-darkblack-400 overflow-hidden min-h-[500px]">
                ...
                {toast && (
                     <div className="fixed top-5 right-5 z-50 flex items-center p-4 rounded-xl shadow-lg border border-bgray-200 dark:border-darkblack-400 bg-white dark:bg-darkblack-600 transition-all transform animate-slide-in">
                          <div className={`flex items-center space-x-3`}>
                               {toast.type === "success" && (
                                    <span className="w-8 h-8 rounded-lg bg-green-100 dark:bg-green-950/40 flex items-center justify-center text-green-500">
                               ...
                               {toast.type === "error" && (
                                    <span className="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-950/40 flex items-center justify-center text-red-500">
                                         <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                               ...
                               {toast.type === "info" && (
                                    <span className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-950/40 flex items-center justify-center text-blue-500">
                                         <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                                              <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 111.085 1.085l-.041.02-.041-.02a.75.75 0 01-1.085-1.085l.041-.02zM12 7.51h.008v.008H12V7.51z" />
                                         </svg>
                                    </span>
                               )}
                               <span className="text-sm font-bold text-bgray-800 dark:text-white">
                                    {toast.message}
                               </span>
                          </div>
                     </div>
