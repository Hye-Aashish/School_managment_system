...
export default function DesignationManagement() {
     ...
     const deleteDes = async (id: string) => {
               ...
               <div className="2xl:flex 2xl:space-x-8">
                    ...
                    <section className="2xl:w-[400px] shrink-0">
                         <div className="bg-white dark:bg-darkblack-600 rounded-2xl p-6 shadow-sm border border-bgray-200 dark:border-darkblack-400">
                              <h3 className="text-xl font-bold dark:text-white mb-6 uppercase tracking-tighter">Designation Registry</h3>
                              <form onSubmit={handleSave} className="space-y-4">
                                   <div className="space-y-1.5">
                                        <label className="text-[10px] font-black text-bgray-500 uppercase tracking-widest px-1">Designation Name *</label>
                                        <input required value={name} onChange={e => setName(e.target.value)} className="w-full h-12 bg-bgray-50 dark:bg-darkblack-500 rounded-xl px-4 text-xs font-bold border-none outline-none focus:ring-2 focus:ring-success-300/30" placeholder="e.g. Senior Teacher" />
                                   </div>
                                   <button type="submit" className="w-full h-12 bg-success-300 text-white font-black rounded-xl hover:bg-success-400 shadow-lg shadow-success-300/20 transition-all uppercase tracking-[0.2em] text-[10px]">SAVE DESIGNATION</button>
                              </form>
                         </div>
                    </section>
                              ...
                              <table className="w-full text-left">
                                   <thead>
                                        <tr className="bg-bgray-50 dark:bg-darkblack-500/30 text-[10px] font-black text-bgray-500 uppercase tracking-widest">
                                             <th className="px-6 py-4">Professional Roles</th>
                                   ...
                                   <tbody className="divide-y divide-bgray-100 dark:divide-darkblack-400">
                                        {loading ? (
                                             ...
                                             designations.map((d) => (
                                                  <tr key={d._id} className="hover:bg-bgray-50/50 transition-colors group">
                                                       ...
                                                       <td className="px-6 py-6 text-right">
                                                            <button onClick={() => deleteDes(d._id)} className="p-2 text-red-400 hover:bg-red-500 hover:text-white rounded-lg transition-all opacity-0 group-hover:opacity-100">
                                             ...
                                             <tr><td colSpan={2} className="py-32 text-center opacity-10 font-black uppercase text-xs tracking-widest">Role Registry Empty</td></tr>
                                        )}
                                   </tbody>
                              </table>
                         </div>
